...
def save_image(img, name, result_dir):
...
def main():
    ...
    save_dir = os.path.join(result_dir, dataset+'_SyN')
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            x_def = ants.apply_transforms(fixed=y,
                                            moving=x,
            ...
            # save image
            save_image(torch.from_numpy(x_def[None, None, ...]), 'deformed_S{}.nii.gz'.format(stdy_idx+2), save_dir)
            save_image(def_seg, 'deformed_seg_S{}.nii.gz'.format(stdy_idx+2), save_dir)
            flow = torch.from_numpy(flow[None, ...])
            print('Flow shape: ', flow.shape)
            save_image(flow.permute(0, 2, 3, 4, 1)[np.newaxis,...], 'flow_S{}.nii.gz'.format(stdy_idx+2), save_dir)
