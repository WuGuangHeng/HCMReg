...
import numpy as np
import modelsv2.configs_MambaMorph as configs
from modelsv2.networks import Unet, ConvBlock
import pdb
import sys
import modelsv2.layers as layers
from modelsv2.mamba import MambaLayer
try:
    from modelsv2.mamba import *
...
if __name__ =='__main__':
    ...
    config = CONFIGS['VMambaMorph']
    model = VMambaMorphFeat(config=config).to('cuda')
    print("11")
    source = torch.randn(1,1,128, 128, 128).cuda()
    target = torch.randn(1,1,128, 128,128).cuda()
    output = model(source, target,True,True)
    print(output[0].shape)
    print(output[1].shape)
    import thop
    macs, params = thop.profile(model, inputs=(target, source))
