...
def main():
    batch_size = 1
    dataset = 'SR_Reg'
    mr_train_dir = dataset + '/Train_mr'
    ct_train_dir = dataset + '/Train_ct'
    mr_val_dir = dataset + '/Val_mr'
    ct_val_dir = dataset + '/Val_ct'

    weights = [1, 1, 0, 0]  # loss weights
    lr = 0.001 # 0.001 for LPBA40
    save_dir = '{}-MambaMorphFeat_ncc_{}_reg_{}_jac_{}_dice_{}_lr_{}/'.format(dataset,
    ...
    train_set = datasets.SRRegBrainDataset(glob.glob(mr_train_dir + '*.pkl'), glob.glob(ct_train_dir + '*.pkl'), transforms=train_composed)
    val_set = datasets.SRRegBrainInferDataset(glob.glob(mr_val_dir + '*.pkl'), glob.glob(ct_val_dir + '*.pkl'), transforms=val_composed)
