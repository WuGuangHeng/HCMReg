...
def main():
    dataset = 'SR_Reg'
    mr_val_dir = dataset + '/Val_mr'
    ct_val_dir = dataset + '/Val_ct'
    weights = [1, 1, 0, 0]  # loss weights
    lr = 0.001
    model_folder = '{}-MambaMorphFeat_ncc_{}_reg_{}_jac_{}_dice_{}_lr_{}/'.format(dataset,
    ...
    if dataset == 'LPBA40':
        ...
        img_size = (160, 192, 192) #(160, 192, 224)
    elif dataset =='MindBoggle':
        img_size = (160, 192, 160)
    else: # SR_Reg
        img_size = (160, 192, 160)
    ...
    test_set = datasets.SRRegBrainInferDataset(glob.glob(mr_val_dir + '*.pkl'), glob.glob(ct_val_dir + '*.pkl'), transforms=test_composed)
    ...
    with torch.no_grad():
        ...
        print('Deformed HD: {:.3f} +- {:.3f}, Raw HD: {:.3f} +- {:.3f}'.format(eval_hd_def.avg,
                                                                                ...
                                                                                eval_hd_raw.std),
                                                                                file=f)
        print('Deformed Det(%): {:.3f} +- {:.3f}'.format(eval_det.avg*100, eval_det.std*100), file=f)
