...
def main():
    dataset = 'MindBoggle' # 'LPBA40', 'OASIS', 'IXI', 'MindBoggle'
    val_dir = dataset+'/test_data'
    label_dir = dataset+'/label_data'
    atlas_dir = dataset+'/fixed.nii.gz'
    weights = [1, 1, 0, 0]  # loss weights
    lr = 0.0001
    model_folder = '{}-AutoFuse_ncc_{}_reg_{}_jac_{}_dice_{}_lr_{}/'.format(dataset,
    ...
    # boxplot file
    boxplot_file = os.path.join(result_dir, dataset + '_AutoFuse_BoxPlot' + '.csv')
    if dataset =='LPBA40':
