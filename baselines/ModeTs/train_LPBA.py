...
from model.RegMamba import Mamba_UNet
from model.AutoFuse import AutoFuse
...
def main():
    ...
    lr = 0.0001 # 0.001 for LPBA40
    save_dir = '{}-AutoFuse_ncc_{}_reg_{}_jac_{}_dice_{}_lr_{}/'.format(dataset,
    ...
    if dataset == 'LPBA40':
        ...
        img_size = (160, 192, 160) #(160, 192, 224)
    else: # MindBoggle
        img_size = (160, 192, 160)
    cont_training = False
    ...
    # RDP
    # model = RDP(img_size, channels=4)

    # RegMamba
    # model = Mamba_UNet(in_channel=2, n_classes=3, start_channel=8, blocks=2)
    
    # CorrMLP
    # model = CorrMLP()
    
    # AutoFuse
    model = AutoFuse(in_channels=1, channel_num=16)
    ...
    train_composed = transforms.Compose(
        [
            trans.CenterCropBySize(img_size), # for IXI
