...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channel=1, first_out_channel=4):
        ...
        self.conv0 = nn.Sequential(
            ConvBlock(in_channel, c), ConvInsBlock(
                c, 2 * c), ConvInsBlock(2 * c, 2 * c)
        )

        self.conv1 = nn.Sequential(
            nn.AvgPool3d(2), ConvInsBlock(
                2 * c, 4 * c), ConvInsBlock(4 * c, 4 * c)
        )

        self.conv2 = nn.Sequential(
            nn.AvgPool3d(2), ConvInsBlock(
                4 * c, 8 * c), ConvInsBlock(8 * c, 8 * c)
        )

        self.conv3 = nn.Sequential(
            nn.AvgPool3d(2), ConvInsBlock(
                8 * c, 16 * c), ConvInsBlock(16 * c, 16 * c)
        )

        self.conv4 = nn.Sequential(
            nn.AvgPool3d(2), ConvInsBlock(
                16 * c, 32 * c), ConvInsBlock(32 * c, 32 * c)
        )
...
class PositionalEncodingLayer(nn.Module):
    def __init__(self, in_channels, dim=6, norm=nn.LayerNorm):
        ...
        self.proj.weight = nn.Parameter(
            Normal(0, 1e-5).sample(self.proj.weight.shape))
...
class ModeTransformer(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=3, qk_scale=None, use_rpb=True):
        ...
        if use_rpb:
            self.rpb = nn.Parameter(
                torch.zeros(self.num_heads, self.rpb_size,
                            self.rpb_size, self.rpb_size)
            )
        vectors = [torch.arange(-s // 2 + 1, s // 2 + 1)
                   for s in [kernel_size] * 3]
        grids = torch.meshgrid(vectors)
    ...
    def makeV(self, N):
        ...
        v = (
            self.grid.reshape(self.kernel_size**3, 3)
            .unsqueeze(0)
            .unsqueeze(0)
            .repeat(N, self.num_heads, 1, 1)
            .unsqueeze(0)
        )
        return v
    ...
    def apply_pb(self, attn, N):
        ...
        return attn + self.rpb.flatten(1, 3)[:, bias_idx].reshape(
            self.num_heads, N, 1, self.rpb_size**3
        ).transpose(0, 1)
    ...
    def forward(self, q, k):
        ...
        q = (
            q.reshape(B, N, self.num_heads, C //
                      self.num_heads, 1).transpose(3, 4)
            * self.scale
        )  # 1, N, heads, 1, head_dim
        ...
        k = (
            k.unfold(1, self.kernel_size, 1)
            .unfold(2, self.kernel_size, 1)
            .unfold(3, self.kernel_size, 1)
            .permute(0, 4, 5, 6, 1, 2, 3)
        )  # C, 3, 3, 3, H, W, T
        k = k.reshape(
            B, self.num_heads, C // self.num_heads, num_tokens, N
        )  # memory boom
...
class ModeT(nn.Module):
    def __init__(
        ...
        self.peblock1 = PositionalEncodingLayer(
            2 * c, dim=head_dim * num_heads[4])
        self.mdt1 = ModeTransformer(
            head_dim * num_heads[4], num_heads[4], qk_scale=scale
        )

        self.peblock2 = PositionalEncodingLayer(
            4 * c, dim=head_dim * num_heads[3])
        self.mdt2 = ModeTransformer(
            head_dim * num_heads[3], num_heads[3], qk_scale=scale
        )

        self.peblock3 = PositionalEncodingLayer(
            8 * c, dim=head_dim * num_heads[2])
        self.mdt3 = ModeTransformer(
            head_dim * num_heads[2], num_heads[2], qk_scale=scale
        )
        self.cwm3 = CWM(3 * num_heads[2], 3 * num_heads[2] * 2)

        self.peblock4 = PositionalEncodingLayer(
            16 * c, dim=head_dim * num_heads[1])
        self.mdt4 = ModeTransformer(
            head_dim * num_heads[1], num_heads[1], qk_scale=scale
        )
        self.cwm4 = CWM(3 * num_heads[1], 3 * num_heads[1] * 2)

        self.peblock5 = PositionalEncodingLayer(
            32 * c, dim=head_dim * num_heads[0])
        self.mdt5 = ModeTransformer(
            head_dim * num_heads[0], num_heads[0], qk_scale=scale
        )
    ...
    def forward(self, moving, fixed):
        ...
        # print("M5.shape", M5.shape, "F5.shape", F5.shape)
        q5, k5 = self.peblock5(F5), self.peblock5(M5)
        # print("q5.shape", q5.shape, "k5.shape", k5.shape)
        w = self.mdt5(q5, k5)
        ...
        # print("q4.shape", q4.shape, "k4.shape", k4.shape)
        w = self.mdt4(q4, k4)
        # print("w.shape", w.shape)
        w = self.cwm4(w)
        # print("w.shape", w.shape)
...
if __name__ == "__main__":
    ...
    model = ModeT(inshape[2:], head_dim=6, num_heads=[8, 4, 2, 1, 1])#.cuda(2)
