...
class AutoFuse(nn.Module):
    def __init__(
        self, in_channels: int = 1, channel_num: int = 16, use_checkpoint: bool = False
    ):
        super().__init__()
        ...
        self.ResizeTransformer = ResizeTransformer_block(
            resize_factor=2, mode="trilinear"
        )
        self.SpatialTransformer = SpatialTransformer_block(mode="bilinear")

    def forward(self, mov_vol, fix_vol):
        ...
        # print("SVF_params", SVF_params.shape)
        # print("warp_vol", warp_vol.shape)
        return warp_vol, flow  # , SVF_params, warp_vol


class AutoFuse_Trans(nn.Module):
    def __init__(
        self, in_channels: int = 1, channel_num: int = 32, use_checkpoint: bool = False
    ):
        super().__init__()

        self.SinglePath = SinglePath_Trans(
            in_channels=in_channels,
            channel_num=channel_num,
            use_checkpoint=use_checkpoint,
        )
        self.FusionPath = FusionPath_Trans(
            in_channels=in_channels * 2,
            channel_num=channel_num * 2,
            use_checkpoint=use_checkpoint,
        )
...
class SinglePath(nn.Module):
    def __init__(
        ...
        self.conv_6 = Conv_block(
            channel_num * 4 + channel_num * 4, channel_num * 4, use_checkpoint
        )
        self.conv_7 = Conv_block(
            channel_num * 4 + channel_num * 2, channel_num * 2, use_checkpoint
        )
        self.conv_8 = Conv_block(
            channel_num * 2 + channel_num * 2, channel_num * 2, use_checkpoint
        )
    ...
    def forward(self, x_in):
        ...
        # print("x_1", x_1.shape) # [1, 16, 144, 192, 160]
        x = self.downsample(x_1)
        x_2 = self.conv_2(x)
        # print("x_2", x_2.shape) # [1, 32, 72, 96, 80]
        x = self.downsample(x_2)
        x_3 = self.conv_3(x)
        # print("x_3", x_3.shape) # [1, 32, 36, 48, 40]
        x = self.downsample(x_3)
        x_4 = self.conv_4(x)
        # print("x_4", x_4.shape) # [1, 64, 18, 24, 20]
        x = self.downsample(x_4)
        x_5 = self.conv_5(x)
        # print("x_5", x_5.shape) # [1, 64, 9, 12, 10]
        x = self.upsample(x_5)
        x = torch.cat([x, x_4], dim=1)
        x_6 = self.conv_6(x)
        # print("x_6", x_6.shape) # [1, 64, 18, 24, 20]
        x = self.upsample(x_6)
        x = torch.cat([x, x_3], dim=1)
        x_7 = self.conv_7(x)
        # print("x_7", x_7.shape) # [1, 32, 36, 48, 40]
        x = self.upsample(x_7)
        x = torch.cat([x, x_2], dim=1)
        x_8 = self.conv_8(x)
        # print("x_8", x_8.shape) # [1, 32, 72, 96, 80]
...
class SinglePath_Trans(nn.Module):
    def __init__(
        ...
        self.PatchEmbed = PatchEmbedding_block(
            patch_size=[2, 2, 2], in_channels=in_channels, embed_dim=channel_num
        )

        self.trans_1 = SwinTrans_stage_block(
        ...
        self.trans_6 = SwinTrans_stage_block(
            embed_dim=channel_num * 2,
            num_layers=2,
            num_heads=channel_num // 4,
            window_size=[5, 5, 5],
        ...
        self.trans_7 = SwinTrans_stage_block(
            embed_dim=channel_num,
            num_layers=2,
            num_heads=channel_num // 8,
            window_size=[5, 5, 5],
    ...
    def forward(self, x_in):
        ...
        x_1 = self.trans_1(x)
        # print("x_1", x_1.shape) # [1, 32, 72, 96, 80]
        x = self.downsample_1(x_1)
        x_2 = self.trans_2(x)
        # print("x_2", x_2.shape) # [1, 64, 36, 48, 40]
        x = self.downsample_2(x_2)
        x_3 = self.trans_3(x)
        # print("x_3", x_3.shape) # [1, 128, 18, 24, 20]
        x = self.downsample_3(x_3)
        x_4 = self.trans_4(x)
        # print("x_4", x_4.shape) # [1, 256, 9, 12, 10]
        x = self.upsample_4(x_4)
        x = torch.cat([x, x_3], dim=1)
        x = self.backdim_5(x)
        x_5 = self.trans_5(x)
        # print("x_5", x_5.shape) # [1, 128, 18, 24, 20]
        x = self.upsample_5(x_5)
        x = torch.cat([x, x_2], dim=1)
        x = self.backdim_6(x)
        x_6 = self.trans_6(x)
        # print("x_6", x_6.shape) # [1, 64, 36, 48, 40]
        x = self.upsample_6(x_6)
        x = torch.cat([x, x_1], dim=1)
        x = self.backdim_7(x)
        x_7 = self.trans_7(x)
        # print("x_7", x_7.shape) # [1, 32, 72, 96, 80]
...
class Integration_block(nn.Module):
    ...
    def forward(self, SVF):
        ...
        for _ in range(self.int_steps):
            ...
            new_locs = new_locs.permute(0, 2, 3, 4, 1)
            new_locs = new_locs[..., [2, 1, 0]]
            flow = flow + nnf.grid_sample(
                flow, new_locs, align_corners=True, mode="bilinear"
            )
...
class Sample_block(nn.Module):
    ...
    def forward(self, mean, log_sigma):
        noise = torch.normal(torch.zeros(mean.shape), torch.ones(mean.shape)).to(
            mean.device
        )
...
class RegHead_block(nn.Module):
    def __init__(self, in_channels: int, use_checkpoint: bool = False):
        ...
        self.mean_head = nn.Conv3d(
            in_channels, 3, kernel_size=3, stride=1, padding="same"
        )
        self.mean_head.weight = nn.Parameter(
            Normal(0, 1e-5).sample(self.mean_head.weight.shape)
        )
        self.mean_head.bias = nn.Parameter(torch.zeros(self.mean_head.bias.shape))

        self.sigma_head = nn.Conv3d(
            in_channels, 3, kernel_size=3, stride=1, padding="same"
        )
        self.sigma_head.weight = nn.Parameter(
            Normal(0, 1e-10).sample(self.sigma_head.weight.shape)
        )
        self.sigma_head.bias = nn.Parameter(
            torch.full(self.sigma_head.bias.shape, -10.0)
        )
...
class FusionGate_block(nn.Module):
    def __init__(self, channel_num: int, use_checkpoint: bool = False):
        super().__init__()
        self.use_checkpoint = use_checkpoint

        self.Conv = nn.Conv3d(
            channel_num, channel_num, kernel_size=3, stride=1, padding="same"
        )
        self.LeakyReLU = nn.LeakyReLU(0.2)
        self.Norm = nn.InstanceNorm3d(channel_num)

        self.Conv_in = nn.Conv3d(
            channel_num * 2, channel_num, kernel_size=1, stride=1, padding="same"
        )
        self.Conv_con = nn.Conv3d(
            channel_num * 2, channel_num, kernel_size=1, stride=1, padding="same"
        )
...
class PatchEmbedding_block(nn.Module):
    def __init__(self, patch_size: list, in_channels: int, embed_dim: int):
        super().__init__()
...
class ReverseEmbedding_block(nn.Module):
    def __init__(self, patch_size: list, embed_dim: int):
        super().__init__()
...
if __name__ == "__main__":
    fix = torch.rand(1, 1, 160, 192, 160)
    mov = torch.rand(1, 1, 160, 192, 160)
    model = AutoFuse(1, 16)
    warp, flow = model(fix, mov)
    print(flow.shape)
