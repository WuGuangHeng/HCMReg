...
from model.Im2Grid import Im2grid
from model.ModeT import ModeT
from model.RDP import RDP
import random
...
def main():
    ...
    lr = 0.001
    model_folder = '{}-RDP_ncc_{}_reg_{}_jac_{}_dice_{}_lr_{}/'.format(dataset,
    ...
    # Im2grid
    # model = Im2grid(img_size, channels=4)
    ...
    save_dir = os.path.join(result_dir, model_folder[:-1])
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            # dice score
            dsc_def = utils.dice_val_VOI(
                def_out.long(), y_seg.long(), dataset=dataset)
            dsc_raw = utils.dice_val_VOI(
            ...
            '''
            save_image(x_def, y, 'deformed_S{}.nii.gz'.format(stdy_idx+2), save_dir)
           
            save_image(flow.permute(0, 2, 3, 4, 1)[np.newaxis, ...], y, 'flow_S{}.nii.gz'.format(stdy_idx+2), save_dir)
            save_image(def_out, y_seg, 'deformed_seg_S{}.nii.gz'.format(stdy_idx+2), save_dir)
            print('Processing {} / {}'.format(stdy_idx, len(test_set)))
            stdy_idx += 1
        '''
        ...
        '''
        print('Average time: {:.3f}'.format(np.mean(TIME)))
