...
file_dir = 'results_analysis/IXI_BoxPlot_Data/'
file_name = ['IXI-UTMorph_ncc_1_reg_1_jac_0_dice_0_lr_0.0001']#['CoTr_ncc_1_diffusion_1', 'PVT_ncc_1_diffusion_1', 'ViTVNet_ncc_1_diffusion_1', 'nnFormer_ncc_1_diffusion_1', 'TransMorphDiff', 'TransMorphBspline_ncc_1_diffusion_1', 'TransMorphBayes_ncc_1_diffusion_1', 'TransMorph_ncc_1_diffusion_1']
...
for exp_name in file_name:
    ...
    exp_data_hd = np.zeros((len(outstruct), 115))
    stct_i = 0
        ...
        with open(file_dir+exp_name+'.csv', "r") as f:
            ...
            for i, line in enumerate(reader):
                if i == 1:
                    ...
                    for lr_i in tar_idx:
                        vals = line[0].split(',')
                        val += float(vals[lr_i].split('-')[0])
                        
                        hd = vals[lr_i].split('-')[1]
                        if hd != 'nan':
                            val_hd += float(hd)
                        else:
                            val_hd += 0
                    ...
                    val = val/len(tar_idx)
                    val_hd = val_hd/hd_num
                    exp_data[stct_i, i-2] = val
                    exp_data_hd[stct_i, i-2] = val_hd

                    #print(stct_i)
        stct_i+=1
    all_dsc.append(exp_data.mean(axis=0))
    all_hd.append(exp_data_hd.mean(axis=0))
    print(exp_data.shape)
    csv_writer(file_dir + file_name[0].split('_')[0] + '_BoxPlot' + '.csv', outstruct, exp_data)

    print("Average DSC: {:.3f}".format(exp_data.mean()))
    print("STD DSC: {:.3f}".format(exp_data.std()))
    print("Average HD: {:.3f}".format(exp_data_hd.mean()))
    print("STD HD: {:.3f}".format(exp_data_hd.std()))
    ...
    print('jec_det(%): {:.6f} +- {:.6f}'.format(my_list.mean(), my_list.std()))
